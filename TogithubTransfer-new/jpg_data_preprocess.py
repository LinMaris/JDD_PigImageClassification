...
def preprocess_all_pics(src_data_dir, dst_data_dir, target_size = [224, 224], random_crop_window = [336, 336], data_plus = 10, train = False):
...
def random_process_image(image, target_size, random_crop_window):
    ...
    if ((h > random_crop_window[0]) and (w > random_crop_window[1])):
        processed_img = random_size_process(processed_img, random_crop_window)
    else:
        resize = min(h, w)
        resize = max(resize, target_size[0]) 
